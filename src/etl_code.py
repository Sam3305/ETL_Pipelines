...
def transform(df, csv_path):
    ...
    exchangeRate_df.set_index('Currency', inplace=True)

    Ex_GBP = exchangeRate_df.loc["GBP", "Rate"]
    Ex_EUR = exchangeRate_df.loc["EUR", "Rate"]
    Ex_INR = exchangeRate_df.loc["INR", "Rate"]

    df['MC_GBP_Billion'] = np.round(df['MC_USD_Billion'] * Ex_GBP, 2)
    df['MC_EUR_Billion'] = np.round(df['MC_USD_Billion'] * Ex_EUR, 2)
    df['MC_INR_Billion'] = np.round(df['MC_USD_Billion'] * Ex_INR, 2)
